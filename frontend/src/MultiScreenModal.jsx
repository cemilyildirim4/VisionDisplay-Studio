  ...
  return (
    <div className="flex items-stretch text-neutral-500 dark:text-neutral-400">
      <button
        ...
        onClick={dec}
        disabled={atMin}
        className={`w-8 h-8 border border-neutral-200 dark:border-[#2c333f] rounded-l-md flex items-center justify-center ${
      ...
      <input
        ...
        onBlur={handleBlur}
        className="w-12 h-8 border-y border-neutral-200 dark:border-[#2c333f] text-center text-sm text-neutral-700 dark:text-neutral-300 focus:outline-none [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none"
      />
      <button
        type="button"
        onClick={inc}
        className="w-8 h-8 border border-neutral-200 dark:border-[#2c333f] rounded-r-lg flex items-center justify-center hover:bg-neutral-50 dark:hover:bg-[#1b2029]"
      >
        <svg viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round">
...
export default function MultiScreenModal({ open, onClose, modelCode, category, initialScreens, onComplete }) {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-[#001334]/45 flex items-center justify-center p-4" onClick={onClose}>
      <div
        className="bg-white dark:bg-[#161a21] rounded-3xl w-full max-w-[1120px] max-h-[92vh] sm:max-h-[92vh] flex flex-col relative shadow-2xl"
      ...
      >
        ...
        <div className="flex items-start justify-between px-8 pt-7 pb-4 shrink-0">
          ...
          <button type="button" onClick={onClose} aria-label={t('exp.close')} className="text-neutral-600 dark:text-neutral-400 hover:text-brand">
        ...
        <div className="flex-1 overflow-y-auto px-4 sm:px-8 pb-2">
          ...
          */}
          <div className="grid grid-cols-2 gap-2.5 sm:gap-5 mb-5 sm:mb-6">
            <CategoryTag label={t('msm.tabLed')} active={!isVideoWall} />
            <CategoryTag label={t('msm.tabVideowall')} active={isVideoWall} />
          </div>

          {/* Seçilen Model */}
          <div className="flex items-center gap-10 mb-6 text-[15px]">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            {screens.map((s, i) => (
              <div key={i} className="border border-neutral-200 dark:border-[#2c333f] rounded-xl p-4">
                <div className="flex items-center justify-between border-b border-neutral-100 dark:border-[#242b36] pb-2.5 mb-3">
                  <div className="flex items-center gap-3">
                    <h3 className="text-base font-bold m-0">{t('screen.label')} {String(i + 1).padStart(2, '0')}</h3>
                    {many && i > 0 && (
                      <button type="button" onClick={() => move(i, -1)} aria-label={t('mse.moveUp')} className="text-neutral-500 dark:text-neutral-400 hover:text-brand">
                    ...
                    {many && i < screens.length - 1 && (
                      <button type="button" onClick={() => move(i, 1)} aria-label={t('mse.moveDown')} className="text-neutral-500 dark:text-neutral-400 hover:text-brand">
                        <svg viewBox="0 0 24 24" width="18" height="18" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">
                          <path d="M12 5v14" />
                  ...
                  {many && (
                    <button type="button" onClick={() => remove(i)} aria-label={t('mse.delete')} className="text-neutral-400 dark:text-neutral-500 hover:text-red-500">
                ...
                <div className="mb-3">
                  ...
                  <div className="grid grid-cols-2 gap-x-3 gap-y-1.5">
                    {TYPES.map((tv) => (
                      <label key={tv} className="flex items-center gap-2 text-sm text-neutral-700 dark:text-neutral-300 cursor-pointer">
                        <input
                          type="radio"
                          name={`type-${i}`}
                          checked={s.type === tv}
          ...
          <div className="flex items-center justify-center gap-6 mt-5 mb-2">
            {many && (
              <button type="button" onClick={reset} className="text-sm text-neutral-700 dark:text-neutral-300 underline hover:text-brand">
                {t('mse.reset')}
              </button>
            )}
          </div>
        ...
        <div className="flex justify-center py-5 shrink-0">
          <button
            ...
            className="rounded-full px-20 py-3.5 font-semibold text-white bg-brand hover:bg-brand-dark transition-colors"
          >
            {t('mse.complete')}
          </button>
        </div>
